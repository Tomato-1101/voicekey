...
- **Mac: ホーム画面に「マイクテスト」「セットアップガイド」の 2 カードを新設（release）**。ホームのダッシュボードに、録音キーを押さずにマイクの入力レベルを確かめられる**マイクテスト**（テスト/停止・ミニレベルメーター・入力デバイス選択）と、セットアップガイドを**もう一度見る**カードを追加した（`UI/HomeView.swift`・`HomeMicTestModel`）。ホーム以外（メニュー/トレイ）へは戻さない。
