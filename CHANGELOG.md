...
- **personal: ダブルタップ（auto_enter）が「たまに検出されない」のを直した（Mac・2026-08-15）**。
  ...
  - **修正**: 判定を純粋型 `DoubleTapPolicy`（`Core/DoubleTapPolicy.swift`）へ切り出し、
    間隔を **押下→押下（press-to-press）** で見るようにした。離鍵→押下だと 1 打目を長く押すほど
    不利になり「速く 2 回叩いた」という体感とずれるため。窓は `NSEvent.doubleClickInterval` に
    追従（下限 0.5 秒・上限 1.2 秒でクランプ）し、press-to-press の許容は `kTapHoldMax + 窓`。
    ホールド上限 `kTapHoldMax = 0.75` は「長い口述の直後に素早く次の録音を始めただけで
    Enter が自動送信される」事故を防ぐガードとしてだけ残した（旧実装の意図をここに集約）。
  - **測れるようにした**: 判定のたびに `.notice` で `slot / hold / gap / press間隔 / 窓 / 結果`
    を残す（不成立は理由つき）。`log.info` は永続化されず `log show` で追えないため。
  - 実測: 旧しきい値なら落ちる 2 ケース（1 打目 hold 0.52 秒／離鍵→押下 0.45 秒）が新実装で成立する
    ことをテストで機械的に確認。`swift build` 成功、`swift test` **147 件 PASS**（新規 15 件・
    `Tests/VoicekeyTests/DoubleTapPolicyTests.swift`）。
