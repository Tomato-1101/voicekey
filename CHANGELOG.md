...
- **Mac: HUD のピル⇄録音インジケーターの変形を「単一カプセルの連続変形」に作り直し（release）**。従来はモード切替のたびにカプセルごと削除＋挿入されて「消して出し直す」感が残っていた。カプセル背景を単一ビューに固定してサイズ（幅・高さ）だけを spring で補間し、中身（mic・波形・字幕・変換マーク）は opacity クロスフェードで出入りする構造へ変更（出現/消滅の transition は HUD 自体の表示・非表示のときだけ）。所要時間は従来と同じ（速度優先の禁則は不変）。
...
- **Mac: 「変換中…」のスピナー（くるくる）を廃止し、waveform マークのゆっくりした明滅パルスに変更（release）**。opacity 1.0⇄0.35＋わずかな scale 揺れ（easeInOut 0.8s 往復）。変換中テキストは維持。transcribing を離れるとアニメーションは確実に停止する（裏で回り続けない）。
...
- **Mac: 待機ピル→録音インジケーターのモーフィングを追加（v3.1・release）**。「ピルを常に表示」ON のとき、録音開始で待機中の小さな横長ピル（mic のみ・薄め）がそのまま大きく育って録音インジケーターへ連続変形し、終了で逆に縮む（同一 HUD パネルを使い回し・hide→show の作り直しをしない）。待機ピルは「本当に小さい横長」に、変形は「大きく育つ」動きをはっきり見せるよう調整。
