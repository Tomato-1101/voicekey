...
- **Google Meet 議事録ボットを追加（2026-08-28）**。会議 URL を渡すと**ボットが裏で会議に入り**、
  ...
  - **文字起こしは全部ローカル計算**（同日ユーザー指示「ミーティング系の文字起こしは全部
    ローカルの計算で。ローカルのモデルで」）。ボットの Chrome が鳴らしている会議音声を
    **PID を固定したシステム音声タップ**で拾い、**Apple のオンデバイス認識**にかける。
    音声もテキストも外へ出ず、API キーも課金も要らない。
    - 当初は Meet の内蔵字幕（＝ Google 側の認識）を読む実装だったが、この指示で差し替えた。
      Meet の DOM を読むのは「いま誰が話しているか」＝議事録の話者名のためだけ
      （取れなければ話者名なしで記録を続ける）。
    ...
    - 記録を始める前にライブ字幕を止める（同じ音を 2 本のタップで拾わない・議事録の二重書き防止）。
