...
- **Mac: HUD のピル⇄録音インジケーターの変形を「単一カプセルの連続変形」に作り直し（release）**。従来はモード切替のたびにカプセルごと削除＋挿入されて「消して出し直す」感が残っていた。カプセル背景を単一ビューに固定してサイズ（幅・高さ）だけを spring で補間し、中身（mic・波形・字幕・変換マーク）は opacity クロスフェードで出入りする構造へ変更（出現/消滅の transition は HUD 自体の表示・非表示のときだけ）。所要時間は従来と同じ（速度優先の禁則は不変）。カプセル背景は**本物のガラス**に刷新: SwiftUI の glassEffect は同一ウィンドウ内の背後しかサンプルできず、他アプリの上に重なる HUD では灰色の不透明な塊になる（構造問題）。ウィンドウ越しに背後のアプリをサンプルできる **AppKit の `NSGlassEffectView`**（macOS 26・`style=.clear`）に差し替え、背後に映り込むものがピルのフチで歪む Liquid Glass の屈折を実機で確認済み。macOS 26 未満と `VOICEKEY_GLASS_FALLBACK=1` は `NSVisualEffectView`（`.behindWindow`）のすりガラスへフォールバック。表示位置も画面下端寄りに変更（浮き 24pt→8pt）。
- **Mac: 録音中の波形バー⇄ストリーミング字幕の切替にアニメーションを追加（release）**。字幕が最初に届いた瞬間にカプセル幅が波形サイズから字幕幅（固定 360pt）へ spring で滑らかに拡張し、以後の文字更新では幅を動かさない（ガタつき防止）。終了時も連続的に縮む。
- **Mac: 「変換中…」のスピナー（くるくる）を廃止し、waveform マークのゆっくりした明滅パルスに変更（release）**。opacity 1.0⇄0.35 の明滅のみ（easeInOut 0.8s 往復・scale は使わない＝マークのサイズは揺れない）。変換中テキストは維持。transcribing を離れるとアニメーションは確実に停止する（裏で回り続けない）。
