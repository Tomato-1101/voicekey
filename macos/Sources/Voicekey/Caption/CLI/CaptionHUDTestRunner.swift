...
enum CaptionHUDTestRunner {
    ...
    static func run(logFilePath: String?) async -> Int32 {
        ...
        writer.write(line(phase: "1行", frame: oneLine, anchor: anchor, onScreen: caption.isOnScreen))
        ...
        writer.write(line(phase: "2行", frame: twoLines, anchor: anchor, onScreen: caption.isOnScreen))
        ...
        try? await Task.sleep(for: .seconds(phaseSeconds))
        writer.write(line(phase: "音声入力中", frame: caption.panelFrame, anchor: anchor, onScreen: caption.isOnScreen))
        ...
        writer.write(line(phase: "復帰", frame: restored, anchor: anchor, onScreen: caption.isOnScreen))
    ...
    /// 1 フェーズぶんの計測行
    private static func line(phase: String, frame: NSRect, anchor: CGPoint, onScreen: Bool) -> String {
        String(
            format: "[HUD] phase=%@ onScreen=%@ bottom=%.1f 期待=%.1f 中心X=%.1f 期待=%.1f 幅=%.1f 高さ=%.1f",
            phase, onScreen ? "yes" : "no",
