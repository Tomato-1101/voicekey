...
- **4 つのバックエンド**: 設定 UI では特徴ベース名「**高精度 / 高速 / 多言語 / リアルタイム**」から選択
  （内部の API 提供元は順に OpenAI / Groq / ElevenLabs (Scribe) / Deepgram。保存値・API キーの紐づけは提供元名のまま）
- **ログイン時自動起動**: 初回起動時に自動登録（設定画面のトグルで解除可能）
- **小型 HUD**: 録音中だけ画面下部に音声レベル連動の波形ピルを表示
- **ダブルタップで自動送信**: ホットキーを素早く 2 回押して録音すると、貼り付け後に Enter を自動送信
- **ハンズフリー切替キー**: 切替キー＋ホットキーで、そのプロバイダーを「押して開始・もう一度で停止」のトグル録音に（既存の長押しは維持）。
  録音中の HUD はティール色の「ハンズフリー」表示になり、「もう一度押すと停止」ヒントが出る。**停止は同じホットキーをもう一度押すだけ**（切替キーは不要）
- **長文の分割並列送信（既定オン）**: 12 秒超の録音を無音区間で分割し API へ並列送信して待ち時間を短縮（語の途中では切らないので精度はそのまま。Deepgram ストリーミングは対象外）
- **JIS（日本語）配列対応**: 記号キーや ¥ / かな / 英数 / 無変換 / 変換 などもホットキーに使える（CGEventTap が物理キーコードで判定）
