...
> **文字起こしモードは 2 種類**: 設定画面では「**リアルタイム**」（最速・録音中に文字起こしが並行して進み、
> キーを離した瞬間に入力）と「**スタンダード**」（録音後にきれいな文章にして入力・おすすめ）から選びます。
> モデルの選択は不要で、それぞれ最適なモデルを自動で使います。整形（読みやすく整える処理）は
> 裏で自動的に行われ、ホットキーごとにオン/オフできます。**新規インストール時の既定は
> 録音キー 1 =「リアルタイム」/ 録音キー 2 =「スタンダード」**。
...
| **リアルタイム** | 最速 | 録音中に文字起こしが並行して進む（Deepgram nova-3・ストリーミング）。キーを離した瞬間に入力されるので待ち時間が最小 |
...
> - **ストリーミング処理**: 「リアルタイム」モードでは録音中に文字起こしを並行実行し、待ち時間を最小化。
> - **録音中の状態表示（HUD）**。
