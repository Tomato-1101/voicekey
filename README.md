...
- macOS 版は、OS 側のオーディオ処理（coreaudiod）が詰まって応答が返らなくなった場合でも
  **「変換中」のまま固まらない**。録音開始が 5 秒、文字起こしが 60 秒（ローカル認識は 20 秒）
  ...
  この状態のあいだ新しい録音は始められないが、**3 秒待っても OS 側が応答しなければ音声エンジンを
  自動で作り直して復帰する**（「オーディオシステムを再起動しました」と表示される。Bluetooth
  イヤホンの接続・切断で詰まったときの主な復帰経路）。作り直しは暴走防止のため 10 分に 3 回までで、
  上限に達すると復帰待ちのままになる。そのときは Mac を再起動するか、ターミナルで
  `sudo killall coreaudiod` を実行する。
