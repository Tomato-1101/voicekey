...
### クリップボードの内容が貼られた / 文字起こし結果がクリップボードに残る

voicekey は日本語を確実に入力するため、**クリップボード経由**で貼り付けます（貼り付け後に
元の内容へ戻します）。

- 貼り付け直後の **約 1 秒間**はクリップボードが文字起こし結果のままです。この間に手動で
  ⌘V / Ctrl+V すると音声の内容が貼られます。1 秒待てば元の内容に戻ります。
- 貼り付け前のクリップボードが**空、または画像などテキスト以外**だった場合、元に戻せないため
  クリップボードは**空**になります（文字起こし結果は残しません）。
- 復元待ちの間にあなたが新しく何かをコピーした場合は、その内容を優先して復元しません。
...
- macOS 版は、OS 側のオーディオ処理（coreaudiod）が詰まって応答が返らなくなった場合でも
  **「変換中」のまま固まらない**。録音開始が 5 秒、文字起こしが 60 秒（ローカル認識は 20 秒）
  ...
  詰まりの引き金だった録音部品（OS 側に毎回「集約デバイス」を作らせる方式）は使うのをやめ、
  **マイクの入力だけを直接受け取る方式に置き換えた**。マイクの準備は一度作ったら使い回し、
  待機中に作り直したり OS 側へ問い合わせを繰り返したりしない
  （待機中はマイクが動かないのでインジケータは点かない）。
  この状態のあいだ新しい録音は始められないが、**3 秒待っても OS 側が応答しなければ voicekey が
  自動で再起動して復帰する**（復帰後に「オーディオの不調から自動復帰しました」と表示される。
  Bluetooth イヤホンの接続・切断で詰まったときの主な復帰経路）。詰まったままのプロセスは
  裏で CPU とメモリを食い続けるため、アプリを作り直すのではなくプロセスごと入れ替えている。
  待機中も 60 秒ごとに点検しているので、ホットキーを押す前に自動で直ることが多い。
  自動再起動は暴走防止のため 30 分に 3 回までで、上限に達すると
  「オーディオが復帰しません。Mac の再起動が必要です」と表示する。そのときは Mac を再起動するか、
  ターミナルで `sudo killall coreaudiod` を実行する。
