...
- **🏠 ホーム画面（Mac）**: 起動時や Dock / Finder から開き直したときに出るダッシュボード。今日／今週／累計の入力量・推定節約時間・レベル／経験値、よく使うアプリの割合、最近の履歴（クリックでコピー）を一覧でき、サイドバーの「設定」を押すと**同じウィンドウのまま設定へ切り替わります**（実績・履歴は設定タブから移設）
