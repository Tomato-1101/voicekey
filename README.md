...
> このセクションはコード／リリース状態が変わるたびに更新します（最終更新: **2026-07-01**）。
>
...
|---|---|---|---|---|
| 🍎 macOS | **v1.7.0** | ✅ **配布中** | [voicekey.vercel.app](https://voicekey.vercel.app) | Apple Development 署名。初回起動のみ Gatekeeper 回避手順あり（サイト／DMG に同梱）。自動更新（Sparkle）対応 |
| 🪟 Windows | **v1.7.0** | ✅ **配布中** | [voicekey.vercel.app](https://voicekey.vercel.app) | GitHub Actions（`windows-build.yml`）でビルド。インストーラ（約 293MB）は容量が大きいため公開バイナリ専用リポ（`voicekey-releases`・ソース非公開）の GitHub Releases でホスト。未署名のため初回のみ SmartScreen 回避手順あり。自動更新対応。インストール時にデスクトップ／スタートメニューへショートカット作成 |
...
- テキスト整形オン/オフ（既定はモード別：**リアルタイム=OFF**〔速度優先〕/ **スタンダード=ON**〔録音後にきれいに整形〕。トグルで変更可）
...
- **文章を自動で整える**: 結果を読みやすく自動整形するか（オン/オフ）。既定はモード別（リアルタイム=OFF / スタンダード=ON）で、文字起こしモードを切り替えるとそのモードの既定に追従します（その後トグルで変更可）。整形のモデル・プロンプトはサーバー側で固定です
