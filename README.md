...
> このセクションはコード／リリース状態が変わるたびに更新します（最終更新: **2026-06-17**）。
...
> **常時 ON に固定（2026-06-17）**: 以下は設定 UI から撤去し、常に有効で動作します（OFF にする項目はありません）。
> - **VAD（無音スキップ）**: 無音時の API 呼び出しをスキップ。
> - **長文の分割並列送信**: 12 秒超の録音を無音区間で分割し並列送信して待ち時間を短縮。
> - **リアルタイムストリーミング**: 「高速リアルタイム」（Deepgram）選択時のみ発火。
> - **録音 HUD 表示**: 録音中の状態表示。
