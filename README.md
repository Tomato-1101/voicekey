...
>
> **このブランチ（`release`）は製品版**です。文字起こしは「高速リアルタイム（Deepgram）/ 正確性（普通入力・Groq）/ 高精度（ハンズフリー・ElevenLabs）」の 3 モード・
...
- **ホットキー 1（普通入力）**: 正確性（Groq）で日常のメモ・チャット（押している間だけ録音）
- **ホットキー 2（ハンズフリー）**: 高精度（ElevenLabs）で重要な議事録（押して開始・もう一度押して停止）

各ホットキーに以下を個別設定可能：

- ショートカットキー（`<F2>`, `<shift_r>`, `<cmd_r>`, `<ctrl>+<space>` 等）
- トリガーモード（`hold`: 押している間録音 / `toggle`: 押して開始/停止）
- 文字起こしモード（**高速リアルタイム** / **正確性** / **高精度**）
...
> **文字起こしモードは 3 種類**: 設定画面では「**高速リアルタイム**」「**正確性**」「**高精度**」から選びます。
> モデルの選択は不要で、それぞれ最適なモデルを自動で使います。整形（読みやすく整える処理）は
> 裏で自動的に行われ、ホットキーごとにオン/オフできます。
> **既定は 普通入力＝正確性（Groq）/ ハンズフリー＝高精度（ElevenLabs）**。話しながら文字を出したいときは
> 「高速リアルタイム」（Deepgram・ストリーミング）を選べます。
...
| スロット | ホットキー | モード | 用途 |
|---|---|---|---|
| ホットキー 1（普通入力） | `<f2>` (hold) | 正確性 | 日常のメモ・チャット（押している間だけ録音・録音後すぐ変換） |
| ホットキー 2（ハンズフリー） | `<f3>` (toggle) | 高精度 | 重要な議事録・正確さ重視（押して開始/停止） |
...
| モード | 速度 | 特徴 |
|---|---|---|
| **正確性** | **最速** | 普通入力の既定。録音後すぐに変換（Groq whisper-large-v3-turbo）。日常のメモ・チャット向き |
| **高精度** | 標準 | ハンズフリーの既定。録音後にまとめて変換（ElevenLabs scribe_v1）。長文や正確さ重視の用途向き |
| **高速リアルタイム** | 最速（ライブ） | 話しながら文字を表示（Deepgram nova-3・ストリーミング）。ライブ字幕的に使いたいとき向き |
...
- **Mode**: `hold` / `toggle`
- **バックエンド**: **高速リアルタイム** / **正確性** / **高精度**（モデルは自動。選択不要）
- **Prompt**: ヒントテキスト（任意）
