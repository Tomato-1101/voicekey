...
- **🪟 コンパクトな録音 HUD**: 画面下部中央に小さく出る録音インジケーター（邪魔にならない小型ピル）。状態に応じて**待機（最小）→変換中（小）→録音中（大）の三段階でなめらかに変形**し、Dock／タスクバーの出没に合わせて縦位置も追従します。設定で「ピルを常に表示」にすると待機中も小さなピルを常時表示できます（両 OS・既定オフ・録音開始でそのまま大きく育つ変形付き。Mac ではフルスクリーン中だけ待機ピルを自動で隠します）。Mac では貼り付け先アプリのアイコンをピル左端に表示します
...
- **🏠 ホーム画面（Mac）**: 起動時や Dock / Finder から開き直したときに出るダッシュボード。統計は 3 カード構成 — **累計入力**（累計文字数＋回数・録音時間＋レベル/進捗バー）、**節約できた時間**（推定節約時間＋「映画 n 本ぶん」等の身近な換算）、**この期間**（「今日⇄今週」を切り替えて期間内の文字数・録音時間・回数を表示。選択は次回も維持）。ほかに、よく使うアプリの割合、最近の履歴（クリックでコピー）を一覧でき、**マイクテスト**（録音キーを押さずにマイクが拾えているか確認・入力デバイス選択つき）と**セットアップガイドをもう一度見る**カードも置いています。サイドバーの「設定」を押すと**同じウィンドウのまま設定へ切り替わります**（実績・履歴は設定タブから移設）
- **📜 サイドノッチ（Mac）**: 画面左端の細いスリット。クリックすると音声入力の履歴パネルが開き、上部の検索フィールドで絞り込み（テキスト・アプリ名）、各行をクリックでコピーできます。設定「表示」の「サイドノッチを表示」で切替（既定オン）
- **🕘 入力履歴（Mac）**: 直近 200 件の音声入力をローカルに保存（貼り付け先アプリ・日時つき）。ホーム画面やサイドノッチから再コピーできます。200 件を超えると古いものから自動で消えます（手動の削除操作は不要。統計・アプリ別の使用状況は履歴と独立した累計のため、履歴が消えても減りません）。設定「履歴」でオフにできます（保存はこの端末のみ・クラウド送信なし）
...
- **履歴**: 履歴を保存（オフにするとその後の入力を履歴に残しません。履歴は 200 件を超えると古いものから自動で消えます）
