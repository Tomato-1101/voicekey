...
- **☁️ Mac と履歴を共有（Windows・任意・既定オフ）**: 設定 → 履歴ページのカードから有効化すると、Cloudflare Workers + D1 経由で Mac 版アプリと音声入力履歴を共有します（自分専用・共有トークン 1 本・オフライン中はキューに溜めて自動再送）。詳細は `docs/HISTORY_SYNC.md`。**Mac 側は未実装**（Windows のみ先行実装済み）
...
- **履歴同期（Windows）**: Mac と履歴を共有する（Cloudflare Workers + D1 経由・既定オフ・共有トークン 1 本）。詳細は `docs/HISTORY_SYNC.md`
