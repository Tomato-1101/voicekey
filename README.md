...
- **🤖 Google Meet 議事録ボット（Mac・macOS 26 以降・自分用ビルドのみ・Chrome が必要）**: 会議の URL を渡すと、
  **ボットが裏で会議に入って文字起こしを議事録として保存**します。メニューバーの「Meet 議事録ボット」→「会議に参加…」。
  - **文字起こしはすべてこの Mac の中**（Apple のオンデバイス音声認識）で行います。会議の音声もテキストも
    外部には送りません。API キーも課金も不要です
  - URL 欄はクリップボードの Meet URL を自動で入れます。参加すると `~/Documents/voicekey/transcripts/` に
    追記されます。話者名は会議画面から読めたときだけ添えます（`- 14:32:05 **山田**: 本日の議題は…`）
  - ボットは**マイクとカメラを切って**入り、会議の音を聞いて文字にするだけです
  - 記録を始めるときライブ字幕が動いていたら自動で止めます（同じ音を二重に記録しないため）
