...
- **macOS（権限・ログイン → 体験 → 完了）**: ようこそ → マイク → アクセシビリティ → 入力監視 → ログイン → **体験①即時入力**（例文を録音キーを押しながら読む）→ **体験②ハンズフリー**（トグル録音）→ **体験③文章整形**（フィラーが自動で消える）→ **機能の紹介**（2 つ目の録音キー／履歴／統計／HUD）→ 完了。各権限は画面のボタンから順に許可でき、許可されると自動でチェックが付きます。
...
あとでもう一度見たいときは、メニューバー（macOS）の「**セットアップガイド…**」から開けます（Windows はタスクトレイのアイコンから）。
