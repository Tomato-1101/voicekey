...
>
> **このブランチ（`release`）は製品版**です。文字起こしは「高速（普通入力・Groq）/ 正確性（ハンズフリー・ElevenLabs）」の 2 モード・
...
- **ホットキー 1（普通入力）**: 高速で日常のメモ・チャット（押している間だけ録音）
- **ホットキー 2（ハンズフリー）**: 正確性で重要な議事録（押して開始・もう一度押して停止）
...
> **文字起こしモードは 2 種類**: 設定画面では「**高速**」と「**正確性**」の 2 つから選びます。
...
| スロット | ホットキー | モード | 用途 |
|---|---|---|---|
| ホットキー 1（普通入力） | `<f2>` (hold) | 高速 | 日常のメモ・チャット（押している間だけ録音） |
| ホットキー 2（ハンズフリー） | `<f3>` (toggle) | 正確性 | 重要な議事録・正確さ重視（押して開始/停止） |
...
| モード | 速度 | 特徴 |
|---|---|---|
| **高速** | **最速** | 普通入力向け。録音後すぐに変換（Groq whisper）。日常のメモ・チャット向き |
| **正確性** | 標準 | ハンズフリー向け。録音後にまとめて変換（ElevenLabs）。長文や正確さ重視の用途向き |
