...
> このセクションはコード／リリース状態が変わるたびに更新します（最終更新: **2026-07-05**）。
>
> **このブランチ（`release`）は製品版**です。文字起こしは「即時入力（Deepgram）/ スタンダード（既定・Groq）」の 2 モード・
...
> **v1.8.0（2026-07-03）**: **初めての人でも迷わない・きれいな文章が出る大型アップデート**。①**初回セットアップガイドを新設**（両 OS）。初回起動時にウィンドウが自動で開き、マイク／アクセシビリティ／入力監視の権限取得とログイン・使い方までを画面の案内どおりに進めるだけで完了する（既存ユーザーには表示されない）。②**文字起こしモードを 2 択に整理**＝「即時入力」（しゃべり終わった瞬間に全文がまとめて入力される・最速）と「スタンダード」（録音後にきれいな文章にして入力・おすすめ）。スタンダードのハンズフリー録音時は長い録音に強いエンジンへ内部で自動切替。③**テキスト整形を実効化**。「えーと」などのフィラー除去・言い直しの整理・箇条書き化をサーバー側で強化し、スタンダードでは文字起こしと整形を 1 リクエストに統合して追加の待ち時間なしで整形済みの文章が入力される（即時入力は速度優先で整形既定オフ）。④**設定画面の言葉を全面刷新**（「バックエンド」→「文字起こしモード」等、専門用語を排除）。⑤**アプリアイコンを新ブランドロゴに刷新**（両 OS）。詳細は [CHANGELOG](CHANGELOG.md)。
>
...
- **💬 ライブ字幕（Mac・macOS 26 以降・自分用ビルドのみ）**: YouTube ライブなどの**再生音声（英語）をその場で日本語字幕**にして、
  画面下のガラス HUD に出します。メニューバーの「ライブ字幕」または **⌥⌘S** で開始/停止。
  - 設定は **設定ウィンドウの「ライブ字幕」タブ**にまとまっています（自動開始・翻訳エンジンとモデル・
    読み上げ・原文表示・対象アプリ・大きさのリセット・API キーの状態）。メニューバーからも同じ操作ができます
  ...
  - **音声入力（ディクテーション）をしている間は字幕を隠します**。裏では認識と翻訳を続けているので、
    入力が終われば続きからまた出ます
  - 一度でも字幕を開始しておくと、次回からは**起動と同時に自動で字幕が始まります**（メニューでオフにできます）
...
配布版はログインのみで文字起こしできます（キー不要）。**ソースから動かす場合**は、文字起こしに **即時入力（Deepgram）/ 正確性（Groq）/ 高精度（ElevenLabs）** のクラウド API を使うため、使うモードの API キーが必要です：
...
| **アカウント / API キー** | 配布版はログインのみ（無料体験 200 回 → アクティベーションキー）。ソースから動かす場合は文字起こし用 API キー（即時入力=Deepgram / 正確性=Groq / 高精度=ElevenLabs。整形も Groq） |
...
初回起動すると、**セットアップガイド**が自動で開きます。権限・ログインの案内のあと、**実際に例文を声で入力してみる体験ステップ**が続き、うまく入力できると褒めてくれます（読むだけでなく、その場で使い心地を体験できます）。

- **macOS（権限・ログイン → 動作確認 → 体験 → 完了）**: ようこそ → マイク → アクセシビリティ → 入力監視（**1 つずつ順番に**許可）→ ログイン → **マイクテスト**（録音キーを押さずに声を出すとレベルメーターが動く・入力デバイスも選べる）→ **録音キーテスト**（録音キーを押すとキーが光る・文字は入りません）→ **体験①即時入力**（例文を録音キーを押しながら読む）→ **体験②ハンズフリー**（トグル録音）→ **体験③文章整形**（フィラーが自動で消える）→ 完了。案内は**本体ウィンドウの中に全画面**で開き（左＝説明／右＝実際の画面イメージ）、完了するとそのままホーム画面になります。各権限は画面のボタンから順に許可でき（**ポップアップは一度に 1 つだけ**）、許可されると自動でチェックが付きます。
  「入力監視」を許可しても反応しない場合は、案内に従って**アプリを再起動**してください（続きから再開します）。
- **Windows（ログイン → 動作確認 → 体験 → 完了）**: ようこそ → ログイン → **マイクテスト**（声を出すとバーが動く）→ **録音キーテスト**（録音キーを押すとキーが光る・文字は入りません）→ 体験①〜③ → まとめ。案内は Mac と同じ**左＝説明／右＝画面イメージの 2 ペイン**です。Windows は OS の権限確認が不要です。設定の「一般」タブの「**セットアップガイドを開く**」からいつでももう一度見られます（同じタブに**マイクテスト**もあります）。
