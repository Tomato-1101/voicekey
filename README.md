...
- **Mode**: `hold` / `toggle`
- **バックエンド**: **リアルタイム** / **スタンダード**（モデルは自動。選択不要。スタンダードのハンズフリー録音時は内部で高精度エンジンへ自動切替）
- **テキスト整形（LLM）**: 結果を読みやすく自動整形するか（オン/オフ）。既定はモード別（リアルタイム=OFF / スタンダード=ON）で、バックエンドを切り替えるとそのモードの既定に追従します（その後トグルで変更可）。整形のモデル・プロンプトはサーバー側で固定です
