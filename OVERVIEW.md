...
|---|---|---|
| 文字起こしの選択肢 | 4 プロバイダーを**実名表示**（OpenAI / Groq / ElevenLabs / Deepgram） | **3 モード**：高速リアルタイム（Deepgram nova-3・短命トークン直叩き＋ストリーミング）/ 正確性（普通入力の既定＝Groq whisper-large-v3-turbo・プロキシ経由）/ 高精度（ハンズフリーの既定＝ElevenLabs scribe_v1・プロキシ経由） |
