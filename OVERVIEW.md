...
| 機能 | 概要 | Mac | Win |
|---|---|:--:|:--:|
| 初回セットアップガイド | 初回起動時にオンボーディングを自動表示。権限・ログインの後ろに**体験ステップ**（例文を実際に声で入力→成功で褒める・即時入力／ハンズフリー／整形の 3 体験＋機能ツアー）を足した体験型（Mac=権限4＋ログイン＋体験3＋ツアー＋完了／Win=ログイン＋体験3＋ツアー＋完了）。体験は「スキップ」で任意。既存ユーザーには出さない・完了フラグで再表示抑止・メニュー/トレイの「セットアップガイド…」で再表示（詳細は CHANGELOG） | ✅ | ✅ |
