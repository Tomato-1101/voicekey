...
| ライブ字幕（personal のみ・macOS 26+） | 再生中のシステム音声（英語）を認識 → 日本語訳 → 録音ピルの真上に固定したガラス字幕（ピルが上へ育つ見た目・音声入力中は自動で隠れる）。最前面アプリの音だけが既定・翻訳は Apple/Gemini/Groq・⌥⌘S で開始停止・起動時の自動開始は既定 ON（統合元は旧 subglass。詳細は CHANGELOG） | ✅ | ❌ |
