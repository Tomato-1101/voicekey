...
| テキスト整形（Groq） | トグルあり・既定 OFF・モデル/プロンプト選択可 | **モデル/プロンプトはサーバー固定・UI は ON/OFF トグルのみ**。既定は**モード別**（スタンダード=ON / リアルタイム=OFF、バックエンド切替で追従）。スタンダードは**サーバーで STT と統合実行**（単発送信は `format=1` の 1 リクエスト、分割送信は結合後にクライアント整形） |
