...
- 現在地: **2026-07-04 の HUD 継続フィードバック第 3 弾まで消化済み・両ブランチ実装済み・未リリース**
  ...
  実測では同一画面内の変化・フルスクリーン Space への切替・復帰の **3 シナリオすべてライブ追従を確認済み**
  （色替え窓＋ピンクのフルスクリーン窓で検証）。**ユーザーの「反映されない」が現行ビルドでも再発する場合は、
  発生状況（アプリ・フルスクリーンか・外部ディスプレイか）を聞いて再調査すること**。
