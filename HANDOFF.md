...
- 現在地: **Mac v1.7.0 / Windows v1.7.0 リリース完了**（2026-07-01。release＝製品版ブランチ。全フィード 200 検証済み・既存ユーザーへ自動更新配信中）。
...
- 次の一手: **v1.7.0（速度改善一連＝3モード整理＋普通入力Groq＋アップロードFLAC化＋放置後初回根治）は両 OS 配布完了**（2026-07-01・全フィード 200 検証済み・自動更新配信中）。
  実機での確認ポイントは**普通入力（正確性=Groq・右⌘ホールド）の総計レイテンシ**（`[計測]` ログの「文字起こし◯ms／総計◯ms」）が main（自分用）にどれだけ近づいたか。FLAC 化でアップロードが約半分になった効果を見る。まだ整形ぶんの差（release は Groq 整形が既定 ON＝STT の後にもう1往復）が残るので、さらに詰めるなら**サーバー側で STT+整形を1リクエストに統合**（クライアント↔Vercel の往復を1回に）が次のレバー。EL「高精度」の段階B（Scribe v2 Realtime でリレー排除）も選択肢に残る。
