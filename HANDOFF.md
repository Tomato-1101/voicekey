...
### 次の一手（2026-07-03 夜・ユーザー指示の新タスク一式。**新セッションで着手する**と明示指示あり）

ユーザーがクリーン再インストールテストを自分で完走した直後のフィードバック。対象は原則 release 実装 → main 移植（Windows は引き続き後日）。

1. **新規ユーザーの既定ホットキー設定を変更（release）**: 録音キー1（メイン）=**長押し（hold）×リアルタイム**、
   録音キー2（サブ）=**右オプションキー×ハンズフリー（トグル）×スタンダード**。新規インストール時の初期値の話
   （オンボーディング/DEFAULT 設定の両方を確認して反映）。
2. **LLM 整形の全面見直し（release・重要）**: 現行の Groq 整形は「いらないものを消している」＝**内容を削るのが NG**。
   - 整形は**プロンプトだけで文を整える**方針に: 削除・要約をせず原文の内容を全部保持し、音声入力の書き起こしとして
     句読点・表記だけ整える。**全プリセットのプロンプトに「適切に句読点を入れる」指示を含める**。
   - **整形フォーマットを複数パターン用意**（Claude が良いと思うものを数種類設計）し、**設定 UI でユーザーが選べる**ようにする。
     ＝release の「プロンプト選択 UI 撤去」方針の改訂: **プリセット選択は出す**（自由入力プロンプトは引き続き出さない。
     モデルは llama-3.1-8b-instant 固定のまま）。
3. **スタンダードモードの長文並列分割（release）**: 長時間録音を 1 本で送ると遅い。**2〜3 個の音声に分割して並列で
   文字起こし→順序を厳守して結合**する。分割点の設計（VAD の無音境界ベース等）も含めて考えること。速さ最重視・
   入力順序を絶対に間違えない。リアルタイム（ストリーミング）は現状で OK、対象はスタンダード（REST）経路。
4. **HUD ピル⇄録音インジケーターのモーフィング再調整（release→main）**: まだアニメーションが不自然。
   「ピルがそのまま自然に大きくなってインジケーターになり、終わると自然に縮んでピルに戻る」連続変形にする
   （消して出し直す・形が飛ぶ感じを完全に排除。所要時間は変えない＝速度優先の禁則は不変）。
5. **サイト: 他社の待ち時間表記の残存確認（voicekey-site）**: ユーザー報告「他社のものは 5 秒になってるけど長すぎる。
   1.5 秒に」。WaitDemo の OLD_WAIT は 1.5 に修正・本番デプロイ済みのはず（コミット 997f131）なので、
   **本番 LP を実際に見て「5秒」に見える残存箇所（別要素・文言・キャッシュ）を特定**して 1.5 秒に統一する。
6. 上記 Mac 実装分は完了後 main へ移植（サイドバーはアカウント行なし等、従来の適応ルール）。

**マシン状態のメモ（新セッション向け）**: 2026-07-03 夜にユーザーが新規ユーザーとして再インストールテストを完走。
voicekey の UserDefaults / 履歴・統計 / キャッシュ / TCC 権限 / Keychain の Auth・DeviceId は一度全消去→テストで再作成済み
（個人 API キー 4 件は温存）。/Applications に本番サイトの v1.8.0 がインストール済み・ログイン済み。
常用はいつもどおり `macos/dist` の release DIST ビルド（未リリースの大型機能入り）が稼働中。

- 現在地: **Mac 大型機能バッチ実装完了・未リリース（CHANGELOG Unreleased・次版候補）**（2026-07-03。両ブランチ実装済み）。
