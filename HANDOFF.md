...
1. **voicekey-site の本番デプロイ（ユーザー実行待ち・唯一のブロッカー）**: 整形プリセットのサーバー側
   （`lib/format.ts` の 4 プリセット・`preset_id` 受け付け）はコミット・push 済み（`bcbba55`）だが、
   `vercel deploy --prod` が Claude の権限レイヤーで拒否され未反映。ユーザーが
   `cd ~/Project/voicekey-site && vercel deploy --prod` を実行するまで、アプリが送る `preset_id` は
   旧サーバーで無視され従来プロンプトのまま動く（壊れはしない・退行なし）。
2. **Windows への移植は引き続き後日（ユーザー指示・未着手）**。対象が増加: 大型機能バッチ（Phase A〜C）＋
   今回の整形プリセット UI・ダッシュボード 3 カード・HUD 刷新・長文分割調整・既定ホットキー。
   次版リリースは両 OS 同期が原則なので、リリース前に Windows 分の扱い（移植 or Mac 先行の可否）を判断する。
...
/Applications に本番の v1.8.0。常用は `macos/dist` の release DIST ビルド＝**今回のフィードバック反映版**
（2026-07-04 ビルド・strings localhost=0 確認済み）が稼働中。

- 現在地: **2026-07-04 朝の HUD 再フィードバック 5 点も消化済み・両ブランチ実装済み・未リリース**（release=`43dcced` / main=`a7901e0`・全 push 済み）。
  ...
  ⑤**ガラス塊バグ修正**: glassEffect は同一ウィンドウ内しかサンプルできず HUD では灰色の不透明な塊になる（構造問題・チューニング不可）
  → ブラー非依存の半透明カプセル（windowBackgroundColor 0.82＋細リム・ライト/ダーク追従）へ差し替え。DIST ビルド→再起動→スクショで
