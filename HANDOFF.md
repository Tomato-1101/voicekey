...
- **字幕を節ごとに刻み、ライブ行をハイブリッド化（統合フェーズ 5・personal・2026-08-10）**。
  ユーザー指摘「一文が長すぎて翻訳されるまで時間がかかりすぎる。もっと文を区切れ区切れにして」。
  - 調整値は `TranslationCoordinator` の先頭に集約（上限 48 字 / 節区切り開始 32 字 / 無音 0.45 秒）。
    詳細と「なぜその順序か」は `CLAUDE.md` の「区切りポリシー」節が正本。
  - クラウド選択時のライブ行を **Apple オンデバイス**で出すようにした（`CaptionService.prepareAppleLiveLineIfNeeded`）。
    副次的に、**429 のときの Apple フォールバックが初めて実際に効くようになった**
    （それまではクラウド選択時に共有 `AppleTranslator` が未準備で、落とし先ごと失敗していた）。
  - **踏んだ罠**: 最初の実装は「文末で終わっていればそのまま出す」を長さ判定より**前**に置いていた。
    音声認識は 199 字の 1 文をまるごと 1 件で渡してくるため、これだと刻みが**一切効かない**。
    実測（mock で 1 刻みのまま）で気付いて判定順を入れ替えた。「直したつもり」を実測で潰した例。
  - 実測（44 語・199 字・エンジン = Groq）: 刻み数 1 → 5、確定→最初の日本語 1110ms → 176ms、
    発話終了→最初の訳表示 1.70s → 0.87s、ライブ行の暫定訳 0 件 → 発話中ずっと（音声から 123ms）。
    `[COVERAGE] status=ok` 維持。計測スクリプトは `scratchpad/measure_split.sh`（`MOCK=0` で実エンジン）。
  - **注意（トレードオフ）**: 刻むぶんクラウドへのリクエスト件数が 1 → 5 に増える。
    各リクエストにシステムプロンプトと文脈が付くので**トークン消費は増える**。
    実際 Groq の無料枠で 429（`retry-after=380s`）を踏んだ（Apple へ落ちるので字幕は止まらない）。
...
- ブランチは **1 本運用**（2026-08-23 確定・**2 ブランチ運用を上書き**）: **`personal` のみ**。
  旧 `main` / `release` / `voice-agent` は personal へ統合してアーカイブ済み（GitHub からも削除。
  バックアップは `~/Project/_archive/voicekey-all-branches-2026-08-23.bundle`）。詳細は `CLAUDE.md`
  冒頭「単一ブランチ運用」を参照。製品版の顧客配布・課金運用は終了し、以下の配布物置き場は
  **`voicekey-site` / `voicekey-releases` 側の歴史的記録**として残す。ソース非公開は
  「private リポジトリ ＋ 配布物に .py 平文を含めない」で担保（後者は voicekey.spec の
  datas=('src','src') が根治対象・未完）。
